...
Funcionario *carregar_todos_txt(const char *nome_arquivo, int *out_count) {
    *out_count = 0;
    ...
    while (fgets(linha, sizeof(linha), f)) {
        ...
        if (*out_count >= cap) { cap *= 2; Funcionario *tmp = realloc(vet, sizeof(Funcionario)*cap); if (!tmp) { free(vet); fclose(f); *out_count=0; return NULL; } vet = tmp; }
        vet[(*out_count)++] = ff;
    }
    fclose(f);
    return vet;
    ...
    while (opcao != 0) {
        ...
        switch (opcao) {
            ...
            case 6:
                ...
                break;
            case 7: restaurar_funcionario_txt(); break;
